  ...
  <section>
    <el-card shadow="never" class="toolbar-card">
      <el-form inline>
        <el-form-item label="退款单号">
          <el-input v-model="filters.refundOrderNo" clearable />
        </el-form-item>
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="records" border>
        <el-table-column prop="refund_order_no" label="退款单号" min-width="190" />
        ...
        <el-table-column label="状态" width="120">
          <template #default="{ row }">
            <el-tag :type="refundTagType(row.status)">{{ labelOf(refundStatusOptions, row.status) }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-drawer v-model="detailVisible" :size="drawerSize" title="退款详情">
      <el-descriptions v-if="detail" :column="descriptionColumns" border>
        <el-descriptions-item label="退款单号">{{ detail.refund_order_no }}</el-descriptions-item>
        ...
        <el-descriptions-item label="状态">{{ labelOf(refundStatusOptions, detail.status) }}</el-descriptions-item>
        <el-descriptions-item label="平台退款状态">{{ detail.trade_status || "-" }}</el-descriptions-item>
        <el-descriptions-item label="更新时间">{{ detail.updated_at }}</el-descriptions-item>
      </el-descriptions>
      <h3>退款请求参数</h3>
      <pre>{{ formatJson(detail?.request_payload) }}</pre>
      <h3>平台退款响应</h3>
      <pre>{{ formatJson(detail?.platform_response) }}</pre>
      <h3>退款通知内容</h3>
...
import { fetchRefunds } from "../api/gatewayAdminApi";
import { labelOf, refundStatusOptions, refundTagType } from "../utils/status";
...
async function load() {
  ...
  try {
    const data = await fetchRefunds({
      refundOrderNo: filters.refundOrderNo || undefined,
