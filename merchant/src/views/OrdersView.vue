  ...
  <section>
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="orders" border>
        ...
        <el-table-column prop="customerDisplayName" label="用户昵称" min-width="120">
          <template #default="{ row }">{{ row.customerDisplayName || "-" }}</template>
        </el-table-column>
    ...
    <el-drawer v-model="detailVisible" :size="drawerSize" title="订单详情">
      <el-descriptions v-if="detail" :column="descriptionColumns" border>
        ...
        <el-descriptions-item label="用户昵称">{{ detail.customerDisplayName || "-" }}</el-descriptions-item>
