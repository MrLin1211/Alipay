    ...
    <el-drawer v-model="drawer.show" title="订单详情" :size="drawerSize" class="order-detail-drawer">
      <template v-if="drawer.order">
        ...
        <div class="detail-actions">
          <el-select v-model="drawer.newStatus" placeholder="变更状态" class="detail-status-select">
            <el-option v-for="o in statusActions" :key="o.value" :label="o.label" :value="o.value" />
          </el-select>
          <el-button type="primary" @click="changeStatus">确认变更</el-button>
        </div>
      </template>
    </el-drawer>
  </section>
...
import { fetchProductOrders, fetchProductOrderDetail, updateProductOrderStatus } from "../api/adminApi";
import { ElMessage } from "element-plus";

const filters = reactive({ orderNo: "", status: "", keyword: "", createdAtRange: [] });
const defaultTime = [
...
const drawer = reactive({ show: false, order: null, newStatus: "" });
...
async function changeStatus() {
  ...
  try {
    const updated = await updateProductOrderStatus(drawer.order.orderNo, drawer.newStatus);
