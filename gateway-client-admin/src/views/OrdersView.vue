  ...
  <section>
    <el-card shadow="never" class="toolbar-card">
      <el-form inline>
        ...
        <el-form-item label="第三方交易号">
          <el-input v-model="filters.thirdTradeNo" clearable />
        </el-form-item>
    ...
    <el-card shadow="never">
      <el-table v-loading="loading" :data="orders" border>
        ...
        <el-table-column prop="merchant_order_no" label="业务订单号" min-width="170" />
        <el-table-column prop="subject" label="标题" min-width="150" />
    ...
    <el-drawer v-model="detailVisible" :size="drawerSize" title="订单详情">
      <el-descriptions v-if="detail" :column="descriptionColumns" border>
        ...
        <el-descriptions-item label="业务订单号">{{ detail.merchant_order_no }}</el-descriptions-item>
        ...
        <el-descriptions-item label="业务通知结果">{{ detail.business_notify_result || "-" }}</el-descriptions-item>
    ...
    <el-dialog v-model="refundVisible" title="提交退款" :width="dialogWidth">
      <el-form :model="refundForm" label-position="top">
        <el-form-item label="网关订单号">
          <el-input v-model="refundForm.gatewayOrderNo" disabled />
        </el-form-item>
        <el-form-item label="退款金额">
          <el-input v-model="refundForm.refundAmount" />
        </el-form-item>
        <el-form-item label="退款原因">
          <el-input v-model="refundForm.refundReason" maxlength="128" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="refundVisible = false">取消</el-button>
        <el-button type="danger" :loading="refunding" @click="submitRefund">提交退款</el-button>
      </template>
    </el-dialog>
  </section>
</template>
...
import { createRefund, fetchOrders } from "../api/clientPayApi";
...
const dialogWidth = computed(() => (viewportWidth.value <= 760 ? "calc(100% - 24px)" : "520px"));
const descriptionColumns = computed(() => (viewportWidth.value <= 760 ? 1 : 2));
const refundForm = reactive({
  gatewayOrderNo: "",
  refundAmount: "",
  refundReason: "接入方后台提交退款"
});
...
async function load() {
  ...
  try {
    const data = await fetchOrders({
      ...
      thirdTradeNo: filters.thirdTradeNo || undefined,
...
function canRefund(row) {
  return row.status === "SUCCESS" && row.trade_status !== "TRADE_FINISHED";
}
